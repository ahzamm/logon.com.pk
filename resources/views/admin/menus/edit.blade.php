        ...
        <div class="row">
          <div class="col-md-12">
            <div class="card card-outline card-info">
              ...
              <form action="{{route('front-menus.update',$menu->id)}}" method="POST" enctype="multipart/form-data">
              <!-- /.card-header -->
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                              ...
                              <select name="parent_menu" class="form-control selectList">
                                ...
                                @foreach ($menus as $item)
                                  <option value="{{$item->id}}" {{$menu->parent_id == $item->id?'selected':''}}>{{$item->menu_name}}</option>
                                @endforeach
                              </select>
                              @error('page_title')
                                  <p class="text-danger mt-2 mb-0 text-sm">{{$message}}</p>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                              ...
                              <option value="{{$item->id}}" {{$menu->page_id == $item->id?'selected':''}}>{{$item->name}}</option>
                                @endforeach
                                </select>
                              @error('page')
                                  <p class="text-danger mt-2 mb-0 text-sm">{{$message}}</p>
                          ...
                          <div class="form-group">
                            ...
                            <select name="icon" class="form-control" id="selectIcon">
                              ...
                              @foreach ($icons as $item)
                                    <option value="{{$item}}" {{$menu->icon == $item?'selected':''}}>{{$item}}</option>
                                @endforeach
                              </select>
                            @error('icon')
                                <p class="text-danger mt-2 mb-0 text-sm">{{$message}}</p>
                        ...
                        {{-- <div class="col-md-6">
                            ...
                            <div class="form-group clearfix">
                                ...
                                <div class="icheck-success d-inline">
                                  <input type="checkbox"  {{$menu->status == 1?'checked':''}}  name="status" id="checkboxSuccess1">
